...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.SAPO:
        assert "values" in data.batch, "SAPO advantage requires critic values. Please enable critic."
        assert "entropy_segment_ids" in data.batch, "SAPO advantage requires entropy_segment_ids."
        advantages, returns = core_algos.compute_sapo_advantage_return(
            token_level_rewards=data.batch["token_level_rewards"],
            values=data.batch["values"],
            response_mask=data.batch["response_mask"],
            gamma=gamma,
            lam=lam,
            entropy_segment_ids=data.batch["entropy_segment_ids"],
            config=config,
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
...
class RayPPOTrainer:
    ...
    def _update_actor(self, batch: DataProto) -> DataProto:
        ...
        use_entropy_segment_policy = (
            self.config.algorithm.adv_estimator == AdvantageEstimator.SAPO
            or self.config.actor_rollout_ref.actor.policy_loss.get("use_entropy_segmentation", False)
        )
        if not use_entropy_segment_policy and "entropy_segment_ids" in batch.batch:
            batch.pop(batch_keys=["entropy_segment_ids"])
        # update actor
        batch_td = batch.to_tensordict()
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if bypass_recomputing_logprobs:  # Use `rollout_log_probs`
                        ...
                        with marked_timer("old_log_prob", timing_raw, color="blue"):
                            ...
                            use_entropy_segmentation = (
                                self.config.algorithm.adv_estimator == AdvantageEstimator.SAPO
                                or actor_config.policy_loss.get("use_entropy_segmentation", False)
                                or self.config.critic.get("use_entropy_segmentation", False)
                            )
                            if use_entropy_segmentation:
                                entropy_topk_percent = self.config.algorithm.get("entropy_topk_percent", 30.0)
                                entropy_segment_ids, entropy_seg_end_mask, value_update_mask = (
                                    core_algos.compute_entropy_segment_info(
                                        response_mask=response_masks,
                                        entropys=entropys,
                                        topk_percent=entropy_topk_percent,
                                    )
                                )
                                old_log_prob.batch["entropy_segment_ids"] = entropy_segment_ids
                                old_log_prob.batch["entropy_seg_end_mask"] = entropy_seg_end_mask
                                old_log_prob.batch["value_update_mask"] = value_update_mask
