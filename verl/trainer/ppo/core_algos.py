...
def compute_sapo_advantage_return(
    ...
    """
    Entropy-based segment-level GAE.
    - Use top-k% highest entropy tokens as segment boundaries.
    - Segment advantages are broadcast back to tokens.
    """

    print(f"entropy-based segmentation, topk_percent={topk_percent}")
    ...
    with torch.no_grad():
        ...
        if entropys.shape != values.shape:
            raise ValueError("entropys.shape must match values.shape")

        k_frac = float(topk_percent)
        if k_frac > 1.0:
            k_frac /= 100.0
        ...
        # [bs, gen_len], True means this token is a segment end
        ...
        for b in range(bs):
            ...
            k = max(int(math.ceil(valid_len * k_frac)), 1)
            score = entropys[b, :valid_len]
            ...
            # selected high-entropy tokens are segment ends
            seg_end_mask[b, cand] = True

            # always make the last valid token a segment end
        ...
        for b in range(bs):
            ...
            end_idxs = torch.nonzero(
                seg_end_mask[b, :valid_len],
                as_tuple=False,
            ).squeeze(-1)
            ...
            for i in reversed(range(len(seg_rewards))):
                next_v = seg_values[i + 1] if i < len(seg_rewards) - 1 else 0.0
                delta = seg_rewards[i] + gamma * next_v - seg_values[i]
                last_adv = delta + gamma * lam * last_adv
                seg_advs[i] = last_adv

            for adv, s, e, v in zip(
                seg_advs,
                starts,
                end_idxs.tolist(),
                seg_values,
...
def segment_mean_by_entropy_topk_inclusive(
    ...
    """
    Segment-wise mean with entropy-based top-k segmentation.

    Segment definition:
      - use top-k% highest entropy tokens as segment ends
      - last valid token is always a segment end

    For each segment, replace x values inside the segment
    with the mean value of that segment.
    """
    ...
    if entropys is None:
        raise ValueError("entropys must be provided")
    if entropys.shape != x.shape:
        raise ValueError("entropys.shape must match x.shape")
    ...
    for b in range(bs):
        ...
        k = int((valid_len * k_frac) + 0.999999)
        k = max(k, 1)
        ...
        seg_end_mask = torch.zeros((valid_len,), device=device, dtype=torch.bool)
        seg_end_mask[cand] = True

        # last valid token is always a segment end
        seg_end_mask[valid_len - 1] = True

        seg_start = 0
        for i in range(valid_len):
            if not seg_end_mask[i]:
                continue

            seg_end = i
            seg_slice = slice(seg_start, seg_end + 1)

            seg_mask = response_mask[b, seg_slice].bool()
            if seg_mask.any():
                seg_mean = out[b, seg_slice][seg_mask].mean()
                out[b, seg_slice] = seg_mean
...
def compute_policy_loss(
    ...
    if entropys is not None:
        negative_approx_kl = segment_mean_by_entropy_topk_inclusive(
...
def compute_value_loss(
    ...
    """
    Value loss with entropy-based segment-end-only updates.

    Segment definition:
      - use top-k% highest entropy tokens as segment ends
      - last valid token is always a segment end

    Only segment-end tokens participate in value loss.
    """

    print(f"[ValueLoss] entropy-based segmentation, topk_percent={topk_percent}")

    bs, gen_len = vpreds.shape
    device = vpreds.device

    if entropys is None:
        raise ValueError("entropys must be provided")
    if entropys.shape != vpreds.shape:
        raise ValueError("entropys.shape must match vpreds.shape")
    ...
    seg_end_mask = torch.zeros_like(response_mask, dtype=torch.bool)
    ...
    for b in range(bs):
        ...
        # high-entropy tokens are segment ends
        seg_end_mask[b, cand] = True
