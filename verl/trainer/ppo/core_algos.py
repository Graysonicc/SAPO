...
def _normalize_topk_percent(topk_percent: float) -> float:
    topk_frac = float(topk_percent)
    if topk_frac <= 0:
        raise ValueError("topk_percent must be > 0")
    if topk_frac > 1.0:
        topk_frac /= 100.0
    return min(max(topk_frac, 0.0), 1.0)


def compute_entropy_segment_info(
    response_mask: torch.Tensor,
    entropys: torch.Tensor,
    topk_percent: float = 30.0,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Build entropy-based segment metadata once for SAPO-style losses.

    High-entropy top-k tokens are treated as segment ends, and the last valid
    response token is always included as a segment end.
    """
    if entropys is None:
        raise ValueError("entropys must be provided")
    if entropys.shape != response_mask.shape:
        raise ValueError("entropys.shape must match response_mask.shape")

    bs, gen_len = response_mask.shape
    device = response_mask.device
    topk_frac = _normalize_topk_percent(topk_percent)

    seg_end_mask = torch.zeros((bs, gen_len), device=device, dtype=torch.bool)
    segment_ids = torch.full((bs, gen_len), -1, device=device, dtype=torch.long)
    ...
    for b in range(bs):
        ...
        k = max(int(math.ceil(valid_len * topk_frac)), 1)
        scores = entropys[b, :valid_len]
        if k >= valid_len:
            cand = torch.arange(valid_len, device=device)
        else:
            cand = torch.topk(scores, k=k, largest=True, sorted=False).indices

        cand = torch.unique(cand).sort().values
        seg_end_mask[b, cand] = True
        seg_end_mask[b, valid_len - 1] = True

        seg_id = 0
        seg_start = 0
        for seg_end in torch.nonzero(seg_end_mask[b, :valid_len], as_tuple=False).squeeze(-1).tolist():
            segment_ids[b, seg_start : seg_end + 1] = seg_id
            seg_id += 1
            seg_start = seg_end + 1

    value_update_mask = response_mask.bool() & seg_end_mask
    return segment_ids, seg_end_mask, value_update_mask


def segment_mean_by_ids(
    x: torch.Tensor,
    response_mask: torch.Tensor,
    segment_ids: torch.Tensor,
) -> torch.Tensor:
    """Replace each valid token by the mean of its entropy-defined segment."""
    if segment_ids.shape != x.shape:
        raise ValueError("segment_ids.shape must match x.shape")
    ...
    for b in range(bs):
        valid_segment_ids = torch.unique(segment_ids[b][valid_mask[b]])
        for seg_id in valid_segment_ids.tolist():
            if seg_id < 0:
                continue
            seg_mask = valid_mask[b] & (segment_ids[b] == seg_id)
            if seg_mask.any():
                out[b, seg_mask] = x[b, seg_mask].mean()
...
def compute_sapo_advantage_return(
    ...
    gamma: float,
    lam: float,
    entropy_segment_ids: torch.Tensor,
    config: Optional[AlgoConfig] = None,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Compute entropy-segmented GAE and broadcast segment advantages to tokens."""
    if entropy_segment_ids is None:
        raise ValueError("entropy_segment_ids must be provided")
    if entropy_segment_ids.shape != token_level_rewards.shape:
        raise ValueError("entropy_segment_ids.shape must match token_level_rewards.shape")

    with torch.no_grad():
        bs, gen_len = token_level_rewards.shape
        device = token_level_rewards.device
        dtype = values.dtype
        advantages = torch.zeros((bs, gen_len), device=device, dtype=dtype)
        new_values = torch.zeros((bs, gen_len), device=device, dtype=dtype)
        ...
        for b in range(bs):
            ...
            valid_segment_ids = entropy_segment_ids[b, :valid_len]
            end_idxs = []
            for seg_id in torch.unique(valid_segment_ids):
                seg_id_item = int(seg_id.item())
                if seg_id_item < 0:
                    continue
                seg_positions = torch.nonzero(valid_segment_ids == seg_id_item, as_tuple=False).squeeze(-1)
                if seg_positions.numel() > 0:
                    end_idxs.append(int(seg_positions[-1].item()))

            if not end_idxs or end_idxs[-1] != valid_len - 1:
                end_idxs.append(valid_len - 1)
            ...
            seg_rewards = []
            seg_values = []
            for start, end in zip(starts, end_idxs):
                rewards = token_level_rewards[b, start : end + 1] * response_mask[b, start : end + 1]
                seg_rewards.append(rewards.sum())
                seg_values.append(values[b, end])

            seg_rewards = torch.stack(seg_rewards)
            seg_values = torch.stack(seg_values)

            last_adv = torch.zeros((), device=device, dtype=dtype)
            seg_advs = torch.zeros_like(seg_rewards)
            for i in reversed(range(len(seg_rewards))):
                next_v = seg_values[i + 1] if i < len(seg_rewards) - 1 else torch.zeros((), device=device, dtype=dtype)
                delta = seg_rewards[i] + gamma * next_v - seg_values[i]
                last_adv = delta + gamma * lam * last_adv
                seg_advs[i] = last_adv

            for adv, start, end, value in zip(seg_advs, starts, end_idxs, seg_values):
                advantages[b, start : end + 1] = adv
                new_values[b, start : end + 1] = value
...
def compute_policy_loss_vanilla(
    ...
    entropy_segment_ids: torch.Tensor | None = None,
) -> tuple[torch.Tensor, dict[str, Any]]:
    """
    Compute the clipped policy objective and related metrics for PPO.
    ...
    # Clamp negative_approx_kl for stability
    negative_approx_kl = torch.clamp(negative_approx_kl, min=-20.0, max=20.0)
    if entropy_segment_ids is not None:
        negative_approx_kl = segment_mean_by_ids(
            x=negative_approx_kl,
            response_mask=response_mask,
            segment_ids=entropy_segment_ids,
        )
    ratio = torch.exp(negative_approx_kl)
    ppo_kl = verl_F.masked_mean(-negative_approx_kl, response_mask)
...
def compute_value_loss(
    ...
    loss_agg_mode: str = "token-mean",
    value_update_mask: torch.Tensor | None = None,
):
    """
    ...
    """
    loss_mask = response_mask if value_update_mask is None else value_update_mask
    loss_mask = loss_mask.to(response_mask.dtype)

    vpredclipped = verl_F.clip_by_value(vpreds, values - cliprange_value, values + cliprange_value)
    vf_losses1 = (vpreds - returns) ** 2
    vf_losses2 = (vpredclipped - returns) ** 2
    clipped_vf_losses = torch.max(vf_losses1, vf_losses2)
    vf_loss = 0.5 * agg_loss(loss_mat=clipped_vf_losses, loss_mask=loss_mask, loss_agg_mode=loss_agg_mode)
