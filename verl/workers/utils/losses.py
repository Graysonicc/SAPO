...
def ppo_loss(config: ActorConfig, model_output, data: TensorDict, dp_group=None):
    ...
    if "entropy_segment_ids" in data:
        fields.append("entropy_segment_ids")
    ...
    entropy_segment_ids = data.get("entropy_segment_ids", None)
    ...
    if entropy_segment_ids is not None and loss_mode == "vanilla":
        policy_loss_kwargs["entropy_segment_ids"] = entropy_segment_ids
...
def value_loss(config: CriticConfig, model_output, data: TensorDict, dp_group=None):
    ...
    if "value_update_mask" in data:
        fields.append("value_update_mask")
    data = data.select(*fields).to_padded_tensor()
    values = data["values"]
    returns = data["returns"]
    response_mask = data["response_mask"].to(bool)
    value_update_mask = data.get("value_update_mask", None)
    ...
    vf_loss, vf_clipfrac = compute_value_loss(
        ...
        loss_agg_mode=config.loss_agg_mode,
        value_update_mask=value_update_mask,
    )
